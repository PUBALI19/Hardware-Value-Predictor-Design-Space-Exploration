...
private:
    ...
    uint64_t  free_list_tail;
    bool      free_list_head_phase;
    bool      free_list_tail_phase;
    uint64_t  free_list_size;    //size of free list = number of physical regs - number of logical regs
    ...
    struct active_list_contents 
    {
        ...
        // PROJECT 4: Value Prediction Fields
        bool     val_pred_valid;  // Instruction was predicted
        uint64_t val_pred_value;  // The actual value predicted
    };
    active_list_contents *active_list;
    uint64_t  active_list_head;
    uint64_t  active_list_tail;
    bool      active_list_head_phase;
    bool      active_list_tail_phase;
    ...
    //
    // Each branch checkpoint contains:
    ...
    br_checkpt *branch_checkpts;

    /////////////////////////////////////////////////////////////////////
    // Private functions.
    // e.g., a generic function to copy state from one map to another.
    /////////////////////////////////////////////////////////////////////
    // this function will copy one map table tp another like amt copied to rmt
    void direct_table_copy(uint64_t *dst, const uint64_t *src) 
    {
        for (uint64_t i = 0; i < n_log_regs; i++)
            dst[i] = src[i];
    }

    //this function returns the number of free registers present in the free list
    uint64_t free_regs_fl() const 
    {
        if (free_list_head_phase == free_list_tail_phase) 
	{
            //tail is ahaed of head if head_phase is equal to tail_phase
            return free_list_tail - free_list_head;
	} else 
	{
	    // if phases are not same then it is full initially
            return free_list_size - (free_list_head - free_list_tail);
	}
    }

    //this function will resturn the number of free registers present in active list
    uint64_t free_regs_al() const 
    {
        if (active_list_head_phase == active_list_tail_phase)
            return n_active - (active_list_tail - active_list_head);
        else
            return active_list_head - active_list_tail;
    }

    //this function will count the number of free bits in GBM
    uint64_t free_gbm(); 
public:
    ////////////////////////////////////////
    // Public functions.
   ...
   /////////////////////////////////////////////////////////////////////
    ...
    /////////////////////////////////////////////////////////////////////
    // PROJECT 4: Value Prediction Dispatch
    // This allows the pipeline to provide the prediction during dispatch.
    /////////////////////////////////////////////////////////////////////
    void dispatch_value_prediction(uint64_t AL_index, uint64_t predicted_value);

    //////////////////////////////////////////
    // Functions related to Reg. Read       //
    // and Execute Stages.                  //
    //////////////////////////////////////////
    ...
    // PROJECT 4: Read Predicted Value
    // Used in Register Read stage to get the value for instructions whose 
    // source register was value-predicted.

    void read_predicted_value(uint64_t AL_index, bool &valid, uint64_t &value);
    //////////////////////////////////////////
