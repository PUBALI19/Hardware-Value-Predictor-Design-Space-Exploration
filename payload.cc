...
void payload::split(unsigned int index) {
   ...
   // Copy Value Prediction fields
   buf[index + 1].vp_eligible   = buf[index].vp_eligible;
   buf[index + 1].vp_predicted  = buf[index].vp_predicted;
   buf[index + 1].vp_pred_value = buf[index].vp_pred_value;
   buf[index + 1].vp_vpq_idx    = buf[index].vp_vpq_idx;
...
void payload::dump(pipeline_t *proc, unsigned int index, FILE *file) {
   ...
   // Value Prediction Dumps
   ifprintf(logging_on, file, "VP Eligible: %u\t", buf[index].val_pred_eligible);
   ifprintf(logging_on, file, "VP Confid  : %u\t", buf[index].val_pred_confident);
   ifprintf(logging_on, file, "VP Value   : 0x%" PRIxreg "\t", buf[index].val_pred_value);
   ifprintf(logging_on, file, "VPQ Index  : %u\n", buf[index].vpq_index);
