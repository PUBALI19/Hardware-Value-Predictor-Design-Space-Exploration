...
void pipeline_t::retire(size_t &instret) {
   ...
   if (head_valid && completed) { // AL head exists and completed
	 ...
	 if (VPU.enabled) {
	     ...
	     if (PAY.buf[idx].vp_eligible) {
	         // Get oracle value for measurements
	         int64_t oracle_val = 0;
	         bool good = PAY.buf[idx].good_instruction;
	         if (good) {
	             db_t *actual = get_pipe()->peek(PAY.buf[idx].db_index);
	             if (actual->a_rdst[0].valid)
	                 oracle_val = (int64_t)actual->a_rdst[0].value;
	         }
	         VPU.retire_train(
	             PAY.buf[idx].pc,
	             PAY.buf[idx].vp_predicted,
	             PAY.buf[idx].vp_pred_value,
	             oracle_val,
	             good,
		     PAY.buf[idx].vp_svp_hit
	         );
	     } else {
	         VPU.retire_ineligible();
	     }
	  }
         // VP END
         REN->commit();
         ...
         if (amo || csr) { // Resume the stalled fetch unit after committing a serializing instruction.
            ...
            // VP BEGIN full VPQ rollback before squash_complete
            if (VPU.enabled && val_misp) {
               VPU.full_rollback();
            }
            // VP END
         ...
         // VP BEGIN – on load violation, full rollback of VPQ
         if (VPU.enabled) {
            VPU.full_rollback();
         }
         // VP END
         ...
         // VP BEGIN – on exception, full rollback of VPQ
         if (VPU.enabled) {
            VPU.full_rollback();
         }
         // VP END
