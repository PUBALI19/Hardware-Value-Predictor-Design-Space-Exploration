...
static void help() {
   ...
   fprintf(stderr, "  --MEMLAT=<latency>\tConfigure a fixed miss penalty for a miss in the LLC.\n");
   fprintf(stderr, "  --vp-eligible=<predINTALU>,<predFPALU>,<predLOAD>\tEligibility for value prediction: 1 to predict that instruction class, 0 to skip.\n");
   fprintf(stderr, "  --vp-perf=<0|1>   Select the perfect value predictor (1 to enable).\n");
   //fprintf(stderr, "  --vp-svp=<VPQsize>,<oracleconf>,<#indexbits>,<#tagbits>,<confmax>\tSelect and configure the Stride Value Predictor (SVP).\n");
   fprintf(stderr, "  --vp-svp=<VPQsize>,<oracleconf>,<#indexbits>,<#tagbits>,<confmax>,<fpc>\tSelect and configure the Stride Value Predictor. fpc=1 enables Forward Probabilistic Counters.\n");
   exit(1);
}
...
static void set_vp_eligible(const char *config) {
   int predINTALU, predFPALU, predLOAD;
   if (sscanf(config, "%d,%d,%d", &predINTALU, &predFPALU, &predLOAD) != 3) {
...
static void set_vp_perf(const char *config) {
   int val;
   if (sscanf(config, "%d", &val) != 1) {
      fprintf(stderr, "Incorrect usage of --vp-perf=<0|1>\n");
      exit(-1);
   }
   else {
      std::cout<<"I am in set_vp_perf val ="<<val<<std::endl;
      VP_PERFECT = (val ? true : false);
      VP_ENABLE  = VP_PERFECT;
      std::cout<<"I am in VP_ENABLE ="<<VP_ENABLE<<std::endl;
   }
}

//static void set_vp_svp(const char *config) {
//   uint64_t vpq_size, oracle_conf, index_bits, tag_bits, conf_max;
//   if (sscanf(config, "%lu,%lu,%lu,%lu,%lu",
//              &vpq_size, &oracle_conf, &index_bits, &tag_bits, &conf_max) != 5) {
...
//      SVP_INDEX_BITS= (unsigned int)index_bits;
//      SVP_TAG_BITS  = (unsigned int)tag_bits;
//      SVP_CONF_MAX  = (unsigned int)conf_max;
//   }
//}

static void set_vp_svp(const char *config) {
    uint64_t vpq_size, oracle_conf, index_bits, tag_bits, conf_max, fpc;
    int n = sscanf(config, "%lu,%lu,%lu,%lu,%lu,%lu",
                   &vpq_size, &oracle_conf, &index_bits,
                   &tag_bits, &conf_max, &fpc);
    if (n < 5) {
