...
void pipeline_t::rename2() {
   ...
   // VP BEGIN count VP-eligible instructions for VPQ stall check
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if (PAY.buf[index].C_valid)
         bundle_dst++;
      // FIX_ME #1 END
      // VP BEGIN – count eligible instructions
      if (VPU.enabled) {
         bool is_intalu = IS_INTALU(PAY.buf[index].flags);
         bool is_fpalu  = IS_FPALU(PAY.buf[index].flags);
         bool is_load   = IS_LOAD(PAY.buf[index].flags) && !IS_AMO(PAY.buf[index].flags);
         if (VPU.is_eligible(PAY.buf[index].C_valid, is_intalu, is_fpalu, is_load))
            bundle_vp_eligible++;
      }
      // VP END
   }
   ...
   if (REN->stall_reg(bundle_dst))
      return;
   // FIX_ME #2 END
   // VP BEGIN stall if VPQ doesn't have enough free entries
   if (VPU.enabled && !VPU.perfect && (VPU.vpq_free() < (int)bundle_vp_eligible))
      return;
   // VP END
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // FIX_ME #5 END
         // VP BEGIN save VPQ tail for this branch checkpoint 
         vp_branch_vpq_tail[PAY.buf[index].branch_ID] = VPU.get_vpq_tail();
         vp_branch_vpq_tail_phase[PAY.buf[index].branch_ID] = VPU.get_vpq_tail_phase();
         // VP END
      }

      // VP BEGIN – value prediction at rename
      // Initialize payload VP fields first
      PAY.buf[index].vp_eligible   = false;
      PAY.buf[index].vp_predicted  = false;
      PAY.buf[index].vp_pred_value = 0;
      PAY.buf[index].vp_vpq_idx    = -1;
      ...
      if (VPU.enabled) {
         ...
         bool eligible = VPU.is_eligible(PAY.buf[index].C_valid, is_intalu, is_fpalu, is_load);
         PAY.buf[index].vp_eligible = eligible;

         if (eligible) {
            // Get oracle value (for perfect mode or oracle_conf mode)
            int64_t oracle_val = 0;
            if ((VPU.perfect || VPU.oracle_conf) && PAY.buf[index].good_instruction) {
               db_t *actual = get_pipe()->peek(PAY.buf[index].db_index);
               if (actual->a_rdst[0].valid)
                  oracle_val = (int64_t)actual->a_rdst[0].value;
            }

            vp_prediction_t pred = VPU.rename_predict(
               PAY.buf[index].pc,
               PAY.buf[index].good_instruction,
               oracle_val
            );

            PAY.buf[index].vp_vpq_idx    = pred.vpq_idx;
            PAY.buf[index].vp_predicted  = pred.predicted;
            PAY.buf[index].vp_pred_value = pred.value;
	    PAY.buf[index].vp_svp_hit    = pred.svp_hit; 
         }
      }
      // VP END
   }
