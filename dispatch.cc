...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // VP BEGIN – inject predicted value BEFORE checking source readiness
      // so same-bundle consumers see the ready bit correctly
      if (VPU.enabled && PAY.buf[index].vp_predicted && PAY.buf[index].C_valid) {
          REN->write(PAY.buf[index].C_phys_reg, (uint64_t)PAY.buf[index].vp_pred_value);
	  // AB_PR set the value in the Active List
	  //REN->dispatch_value_prediction(PAY.buf[index].AL_index, (uint64_t)PAY.buf[index].vp_pred_value);
      ...
      // FIX_ME #9 BEGIN
      //if (PAY.buf[index].C_valid && (!VPU.enabled || !PAY.buf[index].vp_eligible))
      if (PAY.buf[index].C_valid && (!VPU.enabled || !PAY.buf[index].vp_predicted))
      ...
      case SEL_IQ:
         ...
         // FIX_ME #10a BEGIN
         IQ.dispatch(index, DISPATCH[i].branch_mask, PAY.buf[index].lane_id, PAY.buf[index].A_valid, A_ready, PAY.buf[index].A_phys_reg, PAY.buf[index].B_valid, B_ready, PAY.buf[index].B_phys_reg, PAY.buf[index].D_valid, D_ready, PAY.buf[index].D_phys_reg, inum_valid, inum_ready, (unsigned int)inum);
         // FIX_ME #10a END
