...
// VP: value-prediction eligibility macros (spec page 2-3)
...
public:
   ...
   // VP: Value Prediction Unit instance (public so pipeline stages can access it)
   vpu_t VPU;

   // VP: Per-branch-ID VPQ tail snapshot, saved at checkpoint creation.
   // Indexed by branch_ID (0..63). Used by partial_rollback() in squash.cc.
   //NOTE: change this to dynamic array and initialise a constructor
   int  vp_branch_vpq_tail[64];
   bool vp_branch_vpq_tail_phase[64];
   ...
   // AB_PR
   bool vp_execute_check(unsigned int index);
