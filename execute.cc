...
bool pipeline_t :: vp_execute_check(unsigned int index) {
   // Only relevant if VP is enabled and instruction is eligible + predicted
   if (!this->VPU.enabled) return false;
   if (!PAY.buf[index].vp_eligible) return false;

   // If there is no entry that was allocated return
   if(PAY.buf[index].vp_vpq_idx == -1) return false;

   int64_t computed = (int64_t)PAY.buf[index].C_value.dw;

   bool mispredicted = this->VPU.execute_check(
      PAY.buf[index].vp_vpq_idx,
      computed,
      PAY.buf[index].vp_predicted,
      PAY.buf[index].vp_pred_value
   );

   if (PAY.buf[index].vp_predicted && mispredicted) {
      // Post value misprediction in Active List (will trigger VR-1 recovery at retire)
...
void pipeline_t::execute(unsigned int lane_number) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
         ...
         if (IS_LOAD(PAY.buf[index].flags)) {
            ...
            if (hit && PAY.buf[index].C_valid) {
               // VP BEGIN 
               // For a VP-predicted load: check prediction, only write PRF if needed.
               bool misp = vp_execute_check(index);
               bool x = PAY.buf[index].vp_predicted;
               bool y = !misp;

               // Wakeup: only call wakeup if instruction was NOT value-predicted
               if (!x) {
               ...
               // Write PRF only if: !predicted OR (predicted AND incorrect)
               if (!x || !y) {
         ...
         if (PAY.buf[index].C_valid) {
            ...
            // Note: No IQ.wakeup() for ALU instructions regardless of VP –
            // wakeup for single-cycle ALU is done in register_read.cc (FIX_ME #11a).
            // For multi-cycle ALU, in FIX_ME #11b.
            // VP END
         }
      }
   ...
   if (depth > 0) {
      ...
      if (Execution_Lanes[lane_number].ex[depth - 1].valid) {
         ...
         // FIX_ME #11b BEGIN
         // VP BEGIN – skip wakeup for VP-predicted instructions
         if (PAY.buf[index].C_valid && !IS_LOAD(PAY.buf[index].flags) && !IS_AMO(PAY.buf[index].flags)) {
            if (!PAY.buf[index].vp_predicted) {
               // Not VP-predicted: normal wakeup
               IQ.wakeup(PAY.buf[index].C_phys_reg, true);
               REN->set_ready(PAY.buf[index].C_phys_reg);
            } else {
               // VP-predicted: ready bit was already set in Dispatch;
               // skip wakeup to avoid the assertion in issue_queue.cc
               // REN->set_ready(PAY.buf[index].C_phys_reg);
            }
         }
         // FIX_ME #11b END
...
void pipeline_t::load_replay() {
   ...
   if (LSU.load_unstall(cycle, index, value)) {
      ...
      if (PAY.buf[index].C_valid) {
         ...
         // FIX_ME #18a
         // VP BEGIN – VP check for replayed loads
         bool misp = vp_execute_check(index);
         bool x = PAY.buf[index].vp_predicted;
         bool y = !misp;

         // Wakeup: only if not VP-predicted (ready bit was set at Dispatch)
         if (!x) {
            IQ.wakeup(PAY.buf[index].C_phys_reg, true);
            REN->set_ready(PAY.buf[index].C_phys_reg);
         }
