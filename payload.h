...
typedef struct {
   ...
   ////////////////////////
   // VP: Value Prediction fields (set by Rename Stage, used through Retire)
   ////////////////////////

   bool     vp_eligible;      // true if this instruction is eligible for value prediction
   bool     vp_predicted;     // true if a confident prediction was made (SVP hit + confident)
   int64_t  vp_pred_value;    // the predicted destination value (valid when vp_predicted == true)
   int      vp_vpq_idx;       // VPQ slot allocated for this instruction (-1 if none/ineligible)
   bool     vp_svp_hit;       // true if there is a SVP hit
