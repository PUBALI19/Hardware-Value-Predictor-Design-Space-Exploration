...
pipeline_t::pipeline_t(
   ...
   // VP: initialize per-branch VPQ tail snapshots to 0
   for (int _vp_i = 0; _vp_i < 64; _vp_i++){
      vp_branch_vpq_tail[_vp_i] = 0;
      vp_branch_vpq_tail_phase[_vp_i] = false;
    }
...
#undef OPEN_LOG_FILE
   ...
   // AB_PR:
   // Initialise the VPU units required variables
   VPU.enabled     = VP_ENABLE;
   VPU.perfect 	   = VP_PERFECT;
   VPU.oracle_conf = VP_ORACLE_CONF;
   VPU.predINTALU  = VP_PRED_INT_ALU;
   VPU.predFPALU   = VP_PRED_FP_ALU;
   VPU.predLOAD    = VP_PRED_LOAD;

   VPU.n_svp_index_bits = SVP_INDEX_BITS;
   VPU.n_svp_tag_bits   = SVP_TAG_BITS;
   VPU.conf_max         = SVP_CONF_MAX;
   VPU.vpq_size         = VPQ_SIZE;
   VPU.use_fpc = VP_FPC;
   VPU.lfsr    = 0xdeadbeef;

   VPU.init();
   VPU.print_config(stats_log);
...
pipeline_t::~pipeline_t() {
   ...
   // VP: print value prediction storage cost and measurements
   if (VPU.enabled) {
      VPU.print_measurements(stats_log);
   }
